...
if (isset($_POST['btn_inscription'])) {
    ...
    if( empty($email)|| empty($mdp)){
       ...
       header("refrech: 0.5,url=connection.php");
    }else{
        //j'appel la connexion à la bdd
        include('conn.php');

        $req_doublon="SELECT *  FROM user WHERE email=? AND mdp=?";
        
        $stmt_doublon =$bdd->prepare($req_doublon);
        $stmt_doublon->execute( [$email]);
        $result_doublon=$stmt_doublon-> fetch ();
        if(resultat_doublon){
           // die("votre compte à été trouvé");
           echo"<script> alert('votre compte à été trouvé')<script>";
            ...
            <div class="row justify-content-center">
                <div class="col-md-6">
                    <form method="post" class="p-2 bg-light rounded shadow">
                        ...
                        <button type="submit" name="btn_connexion"class="btn btn-primary">Submit</button>
