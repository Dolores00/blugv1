<?php
// vérifier si l'utilisateurs à cliqué
if (isset($_POST['btn_inscription'])) {
    die(var_dump($_POST));
    $name= $_POST[ 'F_name'];
    $email= $_POST[ 'F_email'];
    $mdp= $_POST[ 'F_mdp'];
    $role= $_POST[ "simple_user"];
    ...
    if(empty($name) || empty($email)|| empty($mdp)){
       ...
       header("refrech: 0.5,url=");
    }else{
        //j'appel la connexion à la bdd
        include('conn.php');

        $req_doublon="SELECT *  FROM user WHERE emeil=? ";
        
        $stmt_doublon =$pdo->prepare($req_doublon);
        $result_doublon=$stmt_doublon-> fetch ();
        if(resultat_doublon){
            //die("vous avez dejà un compte");

            echo"<script> alert('vous avez déjà un compte')<script>";

       header("refrech: 0.5,url=dashboard_admin.php");
        }else{
            
    // écrire la requette
    $req_ins="INSERT INTO user (full name,email,mdp,role) VALUES (?,?,?,?)";
    $stmt_ins =$bdd->prepare($req_ins);
    $result_ins=$stmt_ins-> Execute ([$name,$email,$mdp,$role]);
    if(resultat_ins){
       // die("inscription effectuée");

       echo"<script> alert('inscription effectuée')<script>";

       header("refrech: 0.5,url=inscription.php");
    }else{
